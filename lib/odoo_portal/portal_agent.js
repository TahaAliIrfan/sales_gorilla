...
// Locate a real Chrome/Chromium binary. The puma/sidekiq processes don't carry
// PUPPETEER_EXECUTABLE_PATH (it's only set for deploy-time cap commands), and
// Puppeteer's bundled Chromium isn't reliably present on the server — so fall
// back to the system Chrome that's installed there. Locally (macOS), none of
// the Linux paths exist, so we let Puppeteer use its bundled Chromium.
function chromePath() {
  const candidates = [
    process.env.PUPPETEER_EXECUTABLE_PATH,
    process.env.GROVER_CHROME_PATH,
...
async function withPage(cookies, fn) {
  ...
  try {
    ...
    // odoo.com serves a stripped, form-less page to the default "HeadlessChrome"
    // user-agent (bot detection), so present a normal desktop Chrome UA.
...
async function main() {
  ...
  try {
    const data = await withPage(cookies, async (page) => {
      switch (action) {
        case "login": {
          // Log in with email + password and return the resulting cookie jar
          // (incl. the httpOnly session_id) so Ruby can store it. This is how
          // the connect flow captures a session without a manual cookie paste.
          await page.goto(`${baseUrl}/web/login`, { waitUntil: "networkidle2" });

          // The login field can render slightly late, and odoo.com markup varies,
          // so wait for any of the known selectors instead of assuming one.
          const loginSels = ['input[name="login"]', "input#login", 'input[name="email"]', 'input[type="email"]'];
          const passSels = ['input[name="password"]', "input#password", 'input[type="password"]'];
          await page
            .waitForFunction(
          ...
          const loginField = await page.evaluate((sels) => sels.find((s) => document.querySelector(s)) || null, loginSels);
          if (!loginField) {
            // Surface what the page actually served so selectors can be fixed.
            const diag = await page.evaluate(() => ({
              url: location.href,
              title: document.title,
              inputs: Array.from(document.querySelectorAll("input")).map((i) => i.name || i.id || i.type).filter(Boolean).slice(0, 25),
            }));
            throw new Error(`login form not found at ${diag.url} (title: "${diag.title}"); inputs seen: [${diag.inputs.join(", ")}]`);
          }
          const passField = await page.evaluate((sels) => sels.find((s) => document.querySelector(s)) || null, passSels);

          await page.type(loginField, payload.email || "", { delay: 15 });
          if (passField) await page.type(passField, payload.password || "", { delay: 15 });
          await Promise.all([
            page.click('button[type="submit"], input[type="submit"], button.btn-primary').catch(() => {}),
            page.waitForNavigation({ waitUntil: "networkidle2" }).catch(() => {}),
          ]);
          const res = await page.goto(`${baseUrl}/my/leads`, { waitUntil: "networkidle2" });
          const loggedIn = !page.url().includes("/web/login") && !!res && res.ok();
          if (!loggedIn) throw new Error("login failed — check the email/password");
          return { logged_in: true, cookies: await page.cookies() };
        }
