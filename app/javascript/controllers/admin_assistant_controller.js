...
// CRM-wide admin assistant. The conversation is held client-side and posted
// back to the server each turn (POST /admin/assistant/chat); the server answers
// via Groq/Llama tool-calling and persists the turn. The saved thread is handed
// to us via the history value so it renders on reload. Read-only: no send
// actions here (unlike the per-customer ai-chat controller).
export default class extends Controller {
  ...
  async reset() {
    if (this.history.length && !confirm("Clear this conversation and start a new one?")) return
    try {
      await fetch("/admin/assistant/reset", {
        method: "DELETE",
        credentials: "same-origin",
        headers: this.jsonHeaders()
      })
    } catch (error) {
      console.error("Admin assistant reset error:", error)
    }
    this.history = []
    this.messagesTarget.querySelectorAll(":scope > div:not([data-admin-assistant-target='empty'])").forEach((el) => el.remove())
